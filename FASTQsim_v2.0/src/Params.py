...
class Params(object):
    #use 8 processors to populate information and perform empirical fit,unless specified otherwise 
    numthreads=8
    ...
    def getRangeForEmpiricalFit(self,inputfile):   
        ...
        lowval=values[0] 
        highval= values[-1] 
        if (highval-lowval)/float(highval) >0.5:
            highval=math.floor(values[-1]*.95)
        

        return lowval,highval

    def __init__(self,numthreads,param_file,backgroundReads,plotCollection):
        ...
        task_queue=Queue() 
        done_queue=Queue() 
        print "starting processes in params" 
        procs=[] 
        ...
        task_queue.put((fitToData,(0,self.readLengths,self.readLengthsRange,gauss_params,3,'readlength')))
        task_queue.put((fitToData,(1,insertsize_tmp,self.insertSizeRange,gauss_params,3,'insertsize_tmp')))
        task_queue.put((fitToData,(2,insertrepeat_tmp,self.insertSizeRange,gauss_params,3,'insertrepeat_tmp')))
        task_queue.put((fitToData,(3,self.insertCount,self.insertPosRange,gauss_params,3,'insertcount')))
        task_queue.put((fitToData,(4,self.delCount,self.delPosRange,gauss_params,3,'delcount')))         
        task_queue.put((fitToData,(5,delsize_tmp,self.delSizeRange,gauss_params,3,'delsize_tmp'))) 
        task_queue.put((fitToData,(6,delrepeat_tmp,self.delSizeRange,gauss_params,3,'delrepeat_tmp')))
        task_queue.put((fitToData,(7,self.mutationCount,self.mutationPosRange,gauss_params,3,'mutationcount')))
        ...
        while True: 
            if done_queue.empty()==False: 
                [src_input,fitDict,fitType]=done_queue.get()
                if (fitDict==None) and (fitType==None):
                    proc_return_data[src_input]+=1 
                    if proc_return_data[src_input]>4: 
                        returned+=1 
                        print "WARNING: 5 failed attempts to perform curve fitting for model #:"+str(src_input)+';aborting attempt and continuing\n'
                    else: 
                        print "case of returned < 8 and none values in done_queue" 
                        if src_index==0: 
                            task_queue.put((fitToData,(0,self.readLengths,self.readLengthsRange,gauss_params,3,'readlength')))
                        elif src_index==1:
                            task_queue.put((fitToData,(1,insertsize_tmp,self.insertSizeRange,gauss_params,3,'insertsize_tmp')))
                ...
                #print "task queue:"+str(task_queue) 
                sleep(1) 
            elif returned < self.numthreads: 
                print "returned < numthreads" 
                sleep(1)
            else:
        ...
        for p in procs: 
            p.terminate() 
        print "terminated curve fitting procs" 
