...
#sums the entries in dict a, b and returns a combined dict 
def mergeDict(a,b): 
    for entry in b: 
        if entry not in a: 
            a[entry]=b[entry] 
...
#stats helpers
#____________________________________________________________
def padRead(read,sizeToPad):
    read=read.lower(); 
    for i in range(len(read),sizeToPad): 
...
def insertSpikedReads(qualSource,insertedList,qualname,qualSourceDict,pathogen_source):
    ...
    counter=0;
    sortedQualKeys=qualSourceDict.keys()
    sortedQualKeys.sort() 
    ...
    for readlength in insertedList.keys(): 
        ...
        for i in range(len(insertedList[readlength])): 
            read = insertedList[readlength][i][0]
            scores=insertedList[readlength][i][1] 
           #scores = qualSourceDict[closestLength][i%numoptions] 
            insert_pos = positions[i] 
            #length of read should match length of scores 
