...
then
    if [[ -z "$blastresults" ]] 
    then
	ref=ref/hs_ref_GRCh38.fa
	echo "WARNING: No reference specified for BLAST. The algorithm will assume a human host background, so the reference \"ref/hs_ref_GRCH38.fa\" will be used. Please provide a different FASTA reference file if the dataset has no human host reads" 
    fi
fi
...
echo "fasta = $fasta"
echo "qual = $qual"
python2.6 src/fastq_extract.py $inputfastq
echo "complete" 
echo "getting read histogram..."

python2.6 src/readHist.py $fasta $plothistogram
echo "complete" 
echo "getting quality histogram..." 
python2.6 src/qualityHist.py $qual $plothistogram
echo "complete" 
...
python2.6 src/msa_from_sam.py $sam_annotated $basename $plothistogram
echo "complete!"
fi
...
then    
	...
	numblocks=`python2.6 src/fasta_blocks.py $fasta $blocksize`
	echo "complete" 
    ...
    python2.6 src/getBlastStats.py $fasta $basename\_parsed.csv $plothistogram
