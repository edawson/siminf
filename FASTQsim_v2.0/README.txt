...
For additional information about the algorithms used in this software, see the accompanying paper "FASTQSim:Platform-Independent Data Characterizationa and In Silico Read Generation for NGS Datasets"
To test FASTQsim on example data, please download the files in: 
sourceforge.net/projects/fastqsim/files/example_data 
This directory contains example input and output files for FASTQcharacterize and FASTQspike. 

These can be used with the script "example.sh", included with the code distribution to verify the algorithm's functionality. 
...
The script can be run in three ways: 
    ...
    sh FASTQcharacterize.sh -input Example.fq -reference homo_sapien.gdna -sam Example.sam 

    b. Using an externally generated BLAST alignment file to perform characterization 

    sh FASTQcharacterize.sh -input Example.fq -reference homo_sapien.gdna -blastresults Example.blast 
...
Outputs: 
All outputs will be stored in the "FASTQsim" directory. The following files will be generated (using the provided Example.fq input file as an example): 

a. Examplesummary.csv -- This is a list of all output characterization files: 

ExamplereadHist.csv
ExamplequalHist.csv
ExampleUsage.csv
ExampleprimerCheck.csv
ExampleposCount.csv
ExampledelCount.csv
ExampledelsByRead.csv
ExampledelSize.csv
ExampleinsertCount.csv
ExampleinsertsByRead.csv
ExampleinsertSize.csv
ExamplemutationCount.csv
ExamplemutationType.csv


b. ExamplereadHist.csv
...
2. FASTQspike.sh
In the provided example, the background file is "Example.fq", the same that was characterized in the above step. An E. coli genome is spiked in 
...
Inputs: 
a. Background FASTQ file to be spiked (Example.fq) If the goal is to generate reads without spiking them into a background dataset, the flag "-nobackground" should be used instead. 
b. List of background characterization files. This is typically the summary.csv file generated by the characterization script (i.e. Examplesummary.csv) The software comes with characterization files ready for common NGS platforms,including IonTorrent, Roche 454, PacBio, and Illumina. To use one of these default characterization files, specify as follows: 
...
Multiple read sources can be used for spiking. Provide inputs c,d,e for each source of spiked reads. Preface these inputs with the "-source" flag for each source. 
...
a. FASTQ file containing the spiked reads generated from the sources (example/Example_Spiked.fq). 
b. FASTQ file containing the full dataset consisting of the background FASTQ file and the spiked reads inserted at random positions (example/Example_Full.fq).
